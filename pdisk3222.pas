...
type
  ...
  TdiskAddressPacket = record {Used in ext BIOS read/write }
   ...
   {Enhanced Drive Space info Structure}
   FDG_STRUCT = record
   ...
   bytesPerSector : word;
  {EDDConfig : Pointer; -- this may be applicable if bufsize is 30 or more}
   end;
...
Procedure GetEnhDriveSize(drive : byte; Var infobuf : FDG_STRUCT);
...
Begin
  ...
  {Move our result to the record structure}
    dosbuf[transfer_mem].get(infobuf,sizeof(infobuf));
end;
